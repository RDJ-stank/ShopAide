...
Agent 拥有 **12 个工具**，覆盖订单查询、物流轨迹、退货管理、智能判责、时效预警、发票管理、政策检索（RAG）和情绪升级。已接入飞书 IM Bot、微信小程序，永久部署在 `https://shopaide.xyz`，支持三通道并发使用。
...
| **微信小程序** | C 端用户 | 扫码打开，文字对话 | 微信扫码 `miniprogram/` |
| **Chainlit Web UI** | 开发者 / 演示 | 浏览器打开，流式逐字输出 | `http://localhost:8000` |

三个通道共享同一套 Agent 引擎——消息通道是独立的适配层，Agent 核心逻辑完全复用。
...
│  ┌──────┴──────────────────────────┐     │
│  │  Cloudflare CDN                  │     │
│  │  https://shopaide.xyz                │←公网│
│  └─────────────────────────────────┘     │
└─────────────────────────────────────────┘
```

> 已配置永久域名 `shopaide.xyz`（Cloudflare Named Tunnel），重启不会变。
